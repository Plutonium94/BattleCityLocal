...
/* Game Elements */
var tank, enemy, tanks= [], murs, mur1, briques = [], tirs = [], niveaux = [], etatJeu = {niveau:0,scores:false,gagnant:false };
var tousLesMurs = [];
var inputStates = {};
	...
	for(var i = 0;  i < this.lignes; i++) {
		...
		for(j=0; j < this.colonnes; j++) {
			...
			if(!milieu) {
				...
				if(Math.round(Math.random()*10)%4 == 0) b = new Pierre(this.x + j*Brique.width, this.y + i*Brique.height, milieu);
			} else {
...
function start() {
	window.speechSynthesis.speak(new SpeechSynthesisUtterance("Shoot the enemy tank to win the level"));
	...
	//var murs2 = [new Mur(100,100,30,5), new Mur(cw*4/5,ch*4/5,2,6), new MurPierre(cw/2, ch/2-100, 5,5 )];
	...
	window.addEventListener('keydown', function(event) {
		switch(event.keyCode) {
			...
			case 13 : inputStates.enter = true; break;
		}
	}, false);
	...
	window.addEventListener('keyup', function(event) {
		switch(event.keyCode) {
			...
			case 13 : inputStates.enter = false; break;
		}
	}, false);
